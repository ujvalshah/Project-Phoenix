...
/**
 * TagFilterBar: Tag filter toolbar component (formerly CategoryFilterBar)
 * 
 * CATEGORY PHASE-OUT: Renamed from CategoryFilterBar to TagFilterBar
 * 
 * STICKY RULE:
...
export const TagFilterBar: React.FC<TagFilterBarProps> = ({
  ...
  const sortedTags = useMemo(() => {
    // Filter out "All" and "Today" from incoming tags to avoid duplicates
    const filteredTags = tags.filter(
      (tag) => tag.label.toLowerCase() !== 'all' && tag.label.toLowerCase() !== 'today'
    );
  ...
  return (
    ...
    >
      {/* Full-bleed tag bar - matches YouTube's unified toolbar approach */}
      {/* REGRESSION CHECK: Tag filter pills must be text-[12px] font-medium - do not change */}
        ...
        */}
        ...
        >
          {sortedTags.map((tag, index) => {
            ...
            const isToday = tag.label === 'Today';
            // Show divider after "Today" (index 1) and before first tag (index 2)
            ...
            return (
              <React.Fragment key={`${tag.label}-${index}`}>
                ...
                <button
                  onClick={(e) => handleSelect(tag.label, e)}
                  className={`
