    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      ...
      {showRemoveConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowRemoveConfirm(false)} />
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 max-w-sm w-full">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Remove nuggets?</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              Remove {selectedIds.length} nugget{selectedIds.length > 1 ? 's' : ''} from &quot;{collection.name}&quot;? The nuggets won&apos;t be deleted, just removed from this collection.
            </p>
