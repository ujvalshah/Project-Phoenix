...
| `created_at` / `createdAt` Mongo (if any) | `created_at` timestamptz | **createdAt note:** `createdAt` has 0% presence under that key. Before PR-15, verify the actual BSON field name in the live collection. If the field is absent entirely, ETL derives `created_at` from the ObjectId timestamp using: `new Date(parseInt(doc._id.toString().slice(0,8), 16) * 1000)`. Document the confirmed field name here once verified. |
