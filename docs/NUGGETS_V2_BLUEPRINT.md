...
| **Repo layout (PMF freeze)** | New Next app lives in **`web/`** (sibling folder, **own `package.json`**). **NOT** repo root, **NOT** `apps/web` monorepo. Existing root (`src/`, `server/`, `vite.config.ts`, root `package.json`) **untouched** until cutover (PR-18). Avoids React/Vite/Next peer-dep skew during build. Monorepo migration is post-cutover, optional, and a CI/tooling concern. |
...
| **`react-hook-form`** | **ALLOWED** in `web/app/admin/**` only. **Banned** in any non-admin route. Public forms (login, contact) use native `<form>` + Server Action + server-side Zod parse. |
...
- **`react-hook-form`** outside `web/app/admin/**`.
