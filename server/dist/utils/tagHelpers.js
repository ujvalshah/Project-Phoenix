 ...
 */
export async function resolveTagNamesToIds(tagNames) {
 ...
 */
export async function resolveTagIdsToNames(tagIds) {
    if (!tagIds || tagIds.length === 0) {
        return [];
    }
    // Convert all to ObjectId for query
    const objectIds = tagIds.map(id => typeof id === 'string' ? new mongoose.Types.ObjectId(id) : id);
    const tags = await Tag.find({ _id: { $in: objectIds } });
    // Create a map for efficient lookup
    const tagMap = new Map(tags.map(t => [t._id.toString(), t.rawName]));
 ...
 */
export async function updateArticleTagsDualWrite(articleId, tagNames) {
