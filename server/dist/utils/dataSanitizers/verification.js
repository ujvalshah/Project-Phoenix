...
export async function verifyDatabaseIntegrity() {
    ...
    try {
        ...
        const invalidCreatorCollections = collections.filter(c => c.creatorId && !validUserIds.has(c.creatorId));
        result.checks.push({
        ...
        const validArticleIds = new Set((await Article.find({}, '_id').lean()).map(a => a._id.toString()));
        let orphanedEntryCount = 0;
        for (const collection of collections) {
            if (collection.entries && Array.isArray(collection.entries)) {
                orphanedEntryCount += collection.entries.filter((e) => e.articleId && !validArticleIds.has(e.articleId)).length;
            }
        }
        result.checks.push({
        ...
        for (const collection of collections) {
            if (collection.followers && Array.isArray(collection.followers)) {
                orphanedFollowerCount += collection.followers.filter((f) => !validUserIds.has(f)).length;
            }
        }
        result.checks.push({
        ...
        const orphanedAuthorArticles = articles.filter(a => a.authorId && !validUserIds.has(a.authorId));
        result.checks.push({
 ...
 */
export function printVerificationResults(result) {
    console.log('\n' + '='.repeat(80));
