...
export async function sanitizeCollections(dryRun = true) {
    const result = {
    ...
    try {
        ...
        for (const collection of collections) {
            ...
            if (collection.followers && Array.isArray(collection.followers)) {
                const validFollowers = collection.followers.filter((followerId) => validUserIds.has(followerId));
            ...
            if (needsUpdate) {
                if (dryRun) {
                    ...
                    try {
                        await Collection.updateOne({ _id: collection._id }, { $set: updates });
...
//# sourceMappingURL=sanitizeCollections.js.map