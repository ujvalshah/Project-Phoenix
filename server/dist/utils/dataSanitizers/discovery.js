 ...
 */
async function getValidArticleIds() {
 ...
 */
export async function discoverCollectionIssues() {
    const issues = [];
    ...
    const sampleCollectionIds = [];
    const sampleCreatorIds = [];
    const sampleFollowerIds = [];
    for (const collection of collections) {
        ...
        if (collection.followers && Array.isArray(collection.followers)) {
            const orphanedFollowers = collection.followers.filter((followerId) => !validUserIds.has(followerId));
        ...
        if (collection.entries && Array.isArray(collection.entries)) {
            const orphanedArticleEntries = collection.entries.filter((entry) => entry.articleId && !validArticleIds.has(entry.articleId));
            const orphanedUserEntries = collection.entries.filter((entry) => entry.addedByUserId && !validUserIds.has(entry.addedByUserId));
 ...
 */
export async function discoverArticleIssues() {
    const issues = [];
    const validUserIds = await getValidUserIds();
 ...
 */
export async function discoverReportIssues() {
    const issues = [];
    const validUserIds = await getValidUserIds();
    const validArticleIds = await getValidArticleIds();
    ...
    const sampleReporterIds = [];
    const sampleRespondentIds = [];
    const sampleTargetIds = [];
    const sampleActionedByIds = [];
 ...
 */
export async function discoverModerationAuditLogIssues() {
    const issues = [];
    const validUserIds = await getValidUserIds();
    const validReportIds = new Set((await Report.find({}, '_id').lean()).map(r => r._id.toString()));
    const logs = await ModerationAuditLog.find({}).lean();
    let orphanedReportRefs = 0;
    let orphanedPerformerRefs = 0;
    const sampleLogIds = [];
    const sampleReportIds = [];
    const samplePerformerIds = [];
 ...
 */
export async function discoverFeedbackIssues() {
    const issues = [];
    const validUserIds = await getValidUserIds();
    const feedbacks = await Feedback.find({}).lean();
    let orphanedUserRefs = 0;
    const sampleFeedbackIds = [];
    const sampleUserIds = [];
 ...
 */
export async function discoverAllIssues() {
