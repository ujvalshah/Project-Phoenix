 ...
 */
export async function getOrCreateBookmark(userId, itemId, itemType = 'nugget') {
    ...
    if (existing) {
        ...
        await Bookmark.updateOne({ _id: existing._id }, { $set: { lastAccessedAt: new Date().toISOString() } });
 ...
 * @returns The default collection's ID
 */
export async function ensureBookmarkInDefaultCollection(bookmarkId, userId) {
    ...
    if (!existingLink) {
        try {
            ...
            await BookmarkCollection.updateOne({ _id: defaultCollectionId }, { $inc: { bookmarkCount: 1 }, $set: { updatedAt: new Date().toISOString() } });
        }
        catch (error) {
 ...
 */
export function assertAllBookmarkFoldersOwned(requestedCollectionIds, ownedFolderDocs) {
...
export async function assignBookmarkToCollections(bookmarkId, userId, collectionIds) {
    ...
    if (toRemove.length > 0) {
        ...
        await BookmarkCollection.updateMany({ _id: { $in: toRemove } }, { $inc: { bookmarkCount: -1 }, $set: { updatedAt: now } });
    }
    // Add new links
    if (toAdd.length > 0) {
        ...
        await BookmarkCollection.updateMany({ _id: { $in: toAdd } }, { $inc: { bookmarkCount: 1 }, $set: { updatedAt: now } });
    }
}
/**
 ...
 */
export async function removeBookmarkFromCollection(bookmarkId, collectionId) {
    ...
    if (result.deletedCount > 0) {
        ...
        await BookmarkCollection.updateOne({ _id: collectionId }, {
            $inc: { bookmarkCount: -1 },
 ...
 */
export async function deleteBookmarkCompletely(bookmarkId, userId) {
    // Verify ownership
    ...
    if (collectionIds.length > 0) {
        await BookmarkCollection.updateMany({ _id: { $in: collectionIds } }, {
            $inc: { bookmarkCount: -1 },
            $set: { updatedAt: new Date().toISOString() }
        });
    }
 ...
 */
export async function getBookmarkCollectionIds(bookmarkId) {
    const links = await BookmarkCollectionLink.find({
        bookmarkId
 ...
 */
export async function getBookmarkCollectionIdsMap(bookmarkIds) {
 ...
 */
export async function getBookmarkStatus(userId, itemId, itemType = 'nugget') {
    const bookmark = await Bookmark.findOne({
        userId,
        itemId,
        itemType
 ...
 * @param collectionId - The collection's ID
 */
export async function recalculateCollectionCount(collectionId) {
...
//# sourceMappingURL=bookmarkHelpers.js.map