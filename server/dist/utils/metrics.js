...
const requestDurationBuckets = new Map();
const requestDurationSumMs = new Map();
const requestDurationCount = new Map();
const appCounter = new Map();
/** Exported for cache / diagnostics middleware route normalization (low cardinality). */
export function normalizeRouteForMetrics(pathValue) {
...
export function incrementAppCounter(name, labels) {
...
export function snapshotAppCounters() {
...
export function httpLatencyBucketLabel(durationMs) {
...
function observeLatency(key, durationMs) {
...
export function hasMetricsToken() {
    const token = getEnv().METRICS_AUTH_TOKEN;
    return typeof token === 'string' && token.trim().length > 0;
}
export function isAuthorizedMetricsRequest(req) {
    const token = getEnv().METRICS_AUTH_TOKEN;
