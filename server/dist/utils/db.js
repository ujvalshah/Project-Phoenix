 ...
 */
function attachMongooseHooksOnce() {
    if (mongooseHooksAttached)
        return;
...
function transformArticle(doc) {
    ...
    try {
        ...
        const article = {
            ...
            tagIds: rest.tagIds ? rest.tagIds.map((id) => id.toString()) : [],
...
export function invalidateTagNameCache() {
    _tagNameCache = null;
 ...
 */
function resolveTagsFromIds(article, tagNameMap) {
 ...
 */
export async function normalizeArticleDoc(doc) {
 ...
 */
export async function normalizeArticleDocs(docs) {
    const tagMap = await getTagNameMap();
    return docs.map(normalizeDoc).filter(Boolean).map((a) => resolveTagsFromIds(a, tagMap));
}
