...
export async function getMigrationMetrics() {
    const [totalArticles, articlesWithTagIds, articlesWithTags, articlesWithBoth, articlesWithNeither] = await Promise.all([
    ...
    const orphanedCount = allTagIds.filter(id => id && !existingTagIdSet.has(id.toString())).length;
    // Count unique tagIds and tags
    const uniqueTagIds = new Set(allTagIds.filter(Boolean).map(id => id.toString()));
...
//# sourceMappingURL=tagMigrationMetrics.js.map