...
import { setNotificationsEnabled, getNotificationsEnabled, getNotificationRuntimeStatus, } from '../services/notificationService.js';
import { sendValidationError, sendUnauthorizedError, sendNotFoundError, sendInternalError, } from '../utils/errorResponse.js';
...
const subscribeSchema = z
    ...
    .refine((data) => (data.platform === 'web' ? !!data.keys : true), { message: 'keys.p256dh and keys.auth are required for web push', path: ['keys'] })
    .refine((data) => (data.platform === 'android' ? !!data.fcmToken : true), { message: 'fcmToken is required for android', path: ['fcmToken'] });
const renewSchema = z
    .object({
    platform: z.enum(['web', 'android', 'ios']),
    endpoint: z.string().url(),
...
export const subscribe = async (req, res) => {
    ...
    if (!result.success) {
        return sendValidationError(res, 'Invalid subscription data', result.error.errors.map(e => ({ path: e.path, message: e.message })));
    }
    ...
    try {
        await PushSubscription.findOneAndUpdate({ userId, endpoint }, {
            userId,
            platform,
            endpoint,
            keys,
        ...
        await PushSubscription.updateMany({ endpoint, userId: { $ne: userId }, active: true }, { active: false, invalidatedReason: 'reassigned_to_another_user' });
...
export const renewSubscriptionFromServiceWorker = async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return sendUnauthorizedError(res);
    const result = renewSchema.safeParse(req.body);
    if (!result.success) {
        return sendValidationError(res, 'Invalid renewal payload', result.error.errors.map((e) => ({ path: e.path, message: e.message })));
    }
    ...
    if (endpoint === previousEndpoint) {
        try {
            await PushSubscription.updateOne({ userId, endpoint, active: true }, { lastSeenAt: new Date(), keys });
    ...
    try {
        ...
        await PushSubscription.updateOne({ _id: previous._id }, { active: false, invalidatedReason: 'rotated_by_pushsubscriptionchange' });
...
export const unsubscribe = async (req, res) => {
    ...
    try {
        await PushSubscription.updateOne({ endpoint, userId }, { active: false, invalidatedReason: 'user_unsubscribed' });
...
export const updatePreferences = async (req, res) => {
    ...
    if (!result.success) {
        return sendValidationError(res, 'Invalid preferences', result.error.errors.map(e => ({ path: e.path, message: e.message })));
    }
    try {
        const update = {};
...
export const markAsRead = async (req, res) => {
    ...
    if (!userId)
        return sendUnauthorizedError(res);
    try {
        const notification = await Notification.findOneAndUpdate({ _id: req.params.id, userId }, { read: true }, { new: true }).lean();
        if (!notification)
            return sendNotFoundError(res, 'Notification not found');
...
export const toggleNotifications = async (req, res) => {
    ...
    if (!result.success) {
        return sendValidationError(res, 'Invalid toggle value', result.error.errors.map(e => ({ path: e.path, message: e.message })));
    }
    try {
...
export const getNotificationStatus = async (_req, res) => {
    try {
...
export const getNotificationDiagnostics = async (_req, res) => {
    try {
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const since1h = new Date(Date.now() - 60 * 60 * 1000);
        const [enabled, totalActiveSubscriptions, totalUsersSubscribed, subscriptionsByPlatform, providerFailures24h, providerFailures1h, stalenessRemovals24h, sentToProvider24h, shownInApp24h, recentFailures,] = await Promise.all([
        ...
        const platformBreakdown = subscriptionsByPlatform.reduce((acc, row) => {
