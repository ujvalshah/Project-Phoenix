...
import { invalidateUserTokenVersionCache, revokeAllRefreshTokensDetailed, upsertUserTokenVersionCache, } from '../services/tokenService.js';
...
export const getPublicUserProfile = async (req, res) => {
    try {
        ...
        const user = await User.findById(req.params.id).select('role profile.displayName profile.username profile.bio profile.avatarUrl profile.avatarColor profile.title profile.company profile.location profile.website profile.twitter profile.linkedin profile.youtube profile.instagram profile.facebook');
        if (!user)
...
export const updateUser = async (req, res) => {
    try {
        ...
        const roleChanged = isAdminOnOther &&
        ...
        if (updateData.profile) {
            Object.assign(updateObj, Object.keys(updateData.profile).reduce((acc, key) => {
                acc[`profile.${key}`] = updateData.profile[key];
                return acc;
            }, {}));
        }
        if (updateData.preferences && typeof updateData.preferences === 'object') {
            Object.keys(updateData.preferences).forEach(key => {
                if (key === 'notifications') {
                    Object.keys(updateData.preferences.notifications || {}).forEach(nKey => {
                        updateObj[`preferences.notifications.${nKey}`] = updateData.preferences.notifications[nKey];
                    });
                }
                else {
                    updateObj[`preferences.${key}`] = updateData.preferences[key];
                }
            });
        }
        const updateOp = { $set: updateObj };
        if (bumpTokenVersion) {
            updateOp.$inc = { tokenVersion: 1 };
        }
        const user = await User.findByIdAndUpdate(userId, updateOp, { new: true, runValidators: true }).select('-password');
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        if (bumpTokenVersion) {
        ...
        if (emailChanged && priorUser) {
            const newEmail = updateObj['auth.email'];
            ...
            void sendVerificationEmail(newEmail, verificationUrl).catch((err) => {
                const requestLogger = createRequestLogger(req.id || 'unknown', authUserId, req.path);
                requestLogger.warn({
            ...
            if (oldEmail && oldEmail !== newEmail) {
                void sendEmailChangedNoticeEmail(oldEmail, { newEmail }).catch((err) => {
                    const requestLogger = createRequestLogger(req.id || 'unknown', authUserId, req.path);
                    requestLogger.warn({
        ...
        if (isAdminOnOther && priorUser) {
            ...
            const roleChanged = updateData.role !== undefined && priorUser.role !== user.role;
            ...
            if ((updateData.name !== undefined || updateData.profile?.displayName !== undefined) &&
            ...
            if (updateData.profile?.username !== undefined &&
            ...
            if (Object.keys(profileFieldDiff).length > 0) {
                const result = await auditAdminAction(req, {
                    ...
                    targetType: 'user',
                    targetId: userId,
                    previousValue: Object.fromEntries(Object.entries(profileFieldDiff).map(([k, v]) => [k, v.from])),
                    newValue: Object.fromEntries(Object.entries(profileFieldDiff).map(([k, v]) => [k, v.to])),
...
export const getPersonalizedFeed = async (req, res) => {
    try {
        ...
        if (authUserId === userId) {
            await User.findByIdAndUpdate(userId, { $set: { 'appState.lastLoginAt': new Date().toISOString() } }, { new: false } // Don't need to return updated document
            );
        }
        res.json({
