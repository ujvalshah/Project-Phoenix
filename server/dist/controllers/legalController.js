...
export const getLegalPageBySlug = async (req, res) => {
    try {
        ...
        const pagePayload = await getOrSetCachedJson(cacheKey, LEGAL_PAGES_CACHE_TTL_SECONDS, async () => {
...
export const updateLegalPage = async (req, res) => {
    try {
        ...
        const page = await LegalPage.findOneAndUpdate({ slug }, { $set: validationResult.data }, { new: true, lean: true });
        if (!page) {
            return res.status(404).json({ message: 'Legal page not found' });
        }
