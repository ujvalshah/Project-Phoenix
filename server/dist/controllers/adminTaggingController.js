 ...
 */
export const exportTagMapping = async (req, res) => {
    ...
    try {
        ...
        const collections = await Collection.find({ type: 'public' })
            ...
            .lean();
        const articleCollections = new Map();
        ...
        const rows = articles.map(article => {
            ...
            const suggestedSubtopics = new Set();
            for (const colName of colNames) {
                const mapping = COLLECTION_TO_TAG_MAP[colName.toLowerCase().trim()];
                if (!mapping)
                    continue;
                if (mapping.formatTag)
                    suggestedFormats.add(mapping.formatTag);
                if (mapping.domainTags) {
                    for (const dt of mapping.domainTags)
                        suggestedDomains.add(dt);
                }
                if (mapping.subtopicTags) {
                    for (const st of mapping.subtopicTags)
                        suggestedSubtopics.add(st);
                }
            }
            ...
            const sourceUrl = primaryLink?.url ||
            ...
            return {
                ...
                current_tags: (article.tagIds || [])
                    .map((tid) => tagNameMap.get(tid.toString()))
                    .filter(Boolean)
                    .join('; '),
 ...
 */
export const importTagMapping = async (req, res) => {
    const requestLogger = createRequestLogger(req.id || 'unknown', req?.user?.userId, req.path);
    try {
        if (!req.file) {
...
//# sourceMappingURL=adminTaggingController.js.map