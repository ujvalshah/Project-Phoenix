...
import { generateAccessToken, generateEmailVerificationToken, verifyEmailVerificationToken, generatePasswordResetToken, verifyPasswordResetToken, getTokenRemainingSeconds, decodeTokenUnsafe, verifyToken, TOKEN_CONFIG, } from '../utils/jwt.js';
...
import { generateRefreshToken, storeRefreshToken, getUserIdFromRefreshToken, validateRefreshToken, rotateRefreshToken, revokeRefreshToken, revokeAllRefreshTokensDetailed, blacklistToken, recordFailedLogin, clearFailedLogins, isAccountLocked, getUserSessions, isTokenServiceAvailable, checkRefreshTokenReuse, isSessionFingerprintMismatch, RedisUnavailableError, upsertUserTokenVersionCache, } from '../services/tokenService.js';
import { z } from 'zod';
import { sendErrorResponse, sendValidationError, sendUnauthorizedError, sendNotFoundError, sendInternalError } from '../utils/errorResponse.js';
...
const isEmailVerificationEnabled = () => getEnv().ENABLE_EMAIL_VERIFICATION;
...
function deriveUsernameSeed(fullName, email) {
    const fullNameSeed = normalizeUsernameSeed(fullName);
    if (fullNameSeed && fullNameSeed !== 'user')
        return fullNameSeed;
    const emailLocalPart = email.split('@')[0] || '';
    return normalizeUsernameSeed(emailLocalPart);
}
async function generateUniqueUsername(baseSeed) {
...
function getZodIssues(error) {
    const issuesFromV4 = error.issues;
    if (Array.isArray(issuesFromV4)) {
        return issuesFromV4;
    }
    const issuesFromLegacy = error.errors;
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (!validationResult.success) {
            ...
            return sendValidationError(res, formattedMessage || 'Validation failed. Please check your input and try again.', errors);
        }
...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            sendAccountExistsEmail(normalizedEmail, loginUrl).catch((err) => {
                createRequestLogger(req.id || 'unknown', undefined, req.path).warn({
        ...
        const interestedCategories = data.preferences?.interestedCategories ??
        ...
        if (isEmailVerificationEnabled() && process.env.RESEND_API_KEY && newUser.auth.provider === 'email') {
            ...
            sendVerificationEmail(newUser.auth.email, verificationUrl).catch((err) => {
                createRequestLogger(req.id || 'unknown', undefined, req.path).warn({
        ...
        if (error.code === 11000) {
            ...
            if (keyPattern['profile.username']) {
                ...
                try {
                    ...
                    const interestedCategories = retryData.preferences?.interestedCategories ??
...
export const refreshAccessToken = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const isRedisError = error instanceof RedisUnavailableError ||
        ...
        const deviceInfo = req.headers['user-agent'] || 'Unknown device';
        const ipAddress = req.ip || req.socket.remoteAddress;
        if (isSessionFingerprintMismatch({ deviceInfo: tokenData.deviceInfo, ipAddress: tokenData.ipAddress }, { deviceInfo, ipAddress })) {
            requestLogger.error({
...
export const logoutAll = async (req, res) => {
    ...
    try {
        ...
        if (!revokeResult.ok) {
            ...
            clearAuthCookies(res);
            return sendErrorResponse(res, 503, 'Could not revoke all sessions right now. Please retry in a moment.', 'SESSION_REVOCATION_UNAVAILABLE');
        }
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        sendPasswordResetEmail(user.auth.email, resetUrl).catch((err) => {
            requestLogger.error({
...
export const resetPassword = async (req, res) => {
    ...
    try {
        ...
        if (!revokeResult.ok) {
            ...
            return sendErrorResponse(res, 503, 'Password updated, but session revocation is temporarily unavailable. Please retry shortly.', 'SESSION_REVOCATION_UNAVAILABLE');
        }
