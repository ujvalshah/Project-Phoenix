...
import { canModifyCollectionMetadata, canModifyCollectionEntriesPolicy, canViewCollectionPolicy, canCreateCollectionOfType, canInspectArticleCollectionMembership, } from '../utils/editorialCollectionPolicy.js';
import { buildApiCacheKey, getOrSetCachedJson } from '../services/apiResponseCacheService.js';
import { COLLECTION_ARTICLES_CACHE_NS, COLLECTION_ARTICLES_CACHE_TTL_SECONDS, COLLECTIONS_LIST_CACHE_NS, COLLECTIONS_LIST_CACHE_TTL_SECONDS, invalidateRedisCollectionReadCaches, logPublicReadCacheBypass, shouldRedisCacheCollectionArticles, shouldRedisCacheCollectionsList, } from '../config/publicReadCache.js';
function scheduleInvalidateCollectionReadCaches() {
...
async function addEntryIfMissing(collectionId, articleId, addedByUserId, addedAt) {
...
export const getCollections = async (req, res) => {
    try {
        ...
        const payload = await getOrSetCachedJson(cacheKey, COLLECTIONS_LIST_CACHE_TTL_SECONDS, buildCollectionsPayload, {
 ...
 */
export const getCollectionsContainingArticle = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const userRole = req.user?.role;
        if (!userId) {
        ...
        const raw = await Collection.find({
            ...
            .sort({ updatedAt: -1 })
            .lean();
        const visible = raw.filter((doc) => canViewCollectionPolicy({ type: String(doc.type), creatorId: doc.creatorId }, userId, userRole));
...
export const createCollection = async (req, res) => {
    try {
        ...
        if (!canCreateCollectionOfType(type, req.user?.role)) {
...
export const updateCollection = async (req, res) => {
    try {
        ...
        const safeUpdateData = { updatedAt: new Date().toISOString() };
        Object.keys(validationResult.data).forEach(key => {
            const value = validationResult.data[key];
        ...
        if (updateData.name !== undefined) {
            ...
            if (targetType === 'private') {
                ...
                const targetParentId = Object.prototype.hasOwnProperty.call(updateData, 'parentId') && updateData.parentId
                    ? updateData.parentId
                    : collection.parentId;
                if (targetParentId) {
        ...
        const updatedCollection = await Collection.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
        ...
        if (error.code === 11000) {
            ...
            if (updateData?.name) {
                ...
                const duplicateQuery = { canonicalName };
                if (targetType === 'private') {
                    duplicateQuery.creatorId = collection?.creatorId;
                    duplicateQuery.type = 'private';
                }
...
export const addEntry = async (req, res) => {
    try {
        ...
        if (updatedCollection.validEntriesCount === undefined ||
            ...
            await Collection.findOneAndUpdate({ _id: req.params.id }, {
                $set: {
...
export const removeEntry = async (req, res) => {
    try {
        ...
        const updatedCollection = await Collection.findOneAndUpdate({ _id: req.params.id }, {
            $pull: {
...
export const addBatchEntries = async (req, res) => {
    try {
        // Validate input
        ...
        const now = new Date().toISOString();
        const buildBulkOps = (collectionId) => validArticleIds.map((articleId) => ({
            updateOne: {
                filter: {
                    _id: collectionId,
                    'entries.articleId': { $ne: articleId },
        ...
        if (collection.parentId) {
            ...
            if (parent &&
                ...
                await Collection.findOneAndUpdate({ _id: collection.parentId }, {
                    $set: {
...
export const removeBatchEntries = async (req, res) => {
    try {
        // Validate input
        const validationResult = batchEntriesSchema.safeParse(req.body);
        if (!validationResult.success) {
...
export const flagEntry = async (req, res) => {
    try {
        ...
        const entry = collection.entries.find((e) => e.articleId === req.params.articleId);
 ...
 */
export const getCollectionArticles = async (req, res) => {
    try {
        ...
        const articleIds = Array.from(new Set(allEntries.map((e) => e.articleId)));
        ...
        if (q && typeof q === 'string' && q.trim().length > 0) {
            ...
            if (articleQuery.$or || articleQuery.$and) {
                articleQuery.$and = [
                    ...(articleQuery.$and || []),
                    ...(articleQuery.$or ? [{ $or: articleQuery.$or }] : []),
                    { $or: searchConditions },
                ];
        ...
        const sortOrder = sortMap[sort] || { publishedAt: -1, _id: -1 };
        ...
        if (useRedisPub) {
            ...
            const payload = await getOrSetCachedJson(cacheKey, COLLECTION_ARTICLES_CACHE_TTL_SECONDS, buildCollectionArticlesPayload, {
        ...
        logPublicReadCacheBypass({
            ...
            namespace: COLLECTION_ARTICLES_CACHE_NS,
            reasonCode: collection.type !== 'public'
