...
import { getUnseenCountForFeed, getUnseenFeedCountsForUser, markFeedSeenForUser, } from '../services/unseenBadgeService.js';
import { sendValidationError, sendUnauthorizedError, sendForbiddenError, sendNotFoundError, sendPayloadTooLargeError, sendInternalError } from '../utils/errorResponse.js';
import { buildApiCacheKey, getOrSetCachedJson, peekCachedParsedJson, persistApiResponseJsonCache, } from '../services/apiResponseCacheService.js';
import { ARTICLES_LIST_CACHE_NS, ARTICLES_LIST_CACHE_TTL_SECONDS, ARTICLES_DETAIL_CACHE_NS, ARTICLES_DETAIL_CACHE_TTL_SECONDS, describeArticlesListRedisBypassReason, invalidateRedisArticleDerivedReadCaches, logPublicReadCacheBypass, shouldUseArticlesListRedisCache, stableSerializeForArticlesCache, } from '../config/publicReadCache.js';
...
function setCachedResult(key, data, total) {
...
function appendAndQueryCondition(query, condition) {
...
function cloneQueryForSearch(baseQuery) {
...
function mergeSearchConditions(baseQuery, searchConditions) {
    const nextQuery = cloneQueryForSearch(baseQuery);
    if (searchConditions.length === 0)
        return nextQuery;
    const existingOr = Array.isArray(nextQuery.$or) && nextQuery.$or.length > 0 ? [...nextQuery.$or] : null;
    if (existingOr) {
        delete nextQuery.$or;
        const existingAnd = Array.isArray(nextQuery.$and) && nextQuery.$and.length > 0 ? [...nextQuery.$and] : [];
...
async function resolveCategoryIds(categoryNames) {
    ...
    try {
        ...
        getLogger().debug({ inputCount: categoryNames.length, resolvedCount: categoryIds.length }, '[Articles] Resolved category names to IDs');
        return categoryIds;
    }
    catch (error) {
...
export const getArticles = async (req, res) => {
    try {
        const { authorId, q, category, categories, tag, sort, collectionId, favorites, unread, formats, timeRange, youtubeOnly, nonYoutubeOnly, 
        // Dimension-based tag filtering (Phase: three-axis taxonomy)
        formatTagIds: rawFormatTagIds, domainTagIds: rawDomainTagIds, subtopicTagIds: rawSubtopicTagIds, 
        // Content stream routing (standard vs Market Pulse)
        contentStream, searchMode, visibility, status, } = req.query;
        ...
        const requestedVisibility = visibility === 'public' || visibility === 'private' ? visibility : undefined;
        ...
        if (q && typeof q === 'string' && q.trim().length > 0) {
            ...
            if (trimmedQ.length >= 3) {
                ...
                const textCondition = { $text: { $search: trimmedQ } };
                const searchConditions = [textCondition];
        ...
        if (category && typeof category === 'string' && category === 'Today') {
            ...
            const hasToday = categoryArray.some((cat) => typeof cat === 'string' && cat === 'Today');
            if (hasToday) {
                ...
                const otherCategories = categoryArray.filter((cat) => typeof cat === 'string' && cat !== 'Today');
                if (otherCategories.length > 0) {
                    query.categories = {
                        $in: otherCategories.map((cat) => createExactMatchRegex(cat))
                    };
                }
            }
            else {
                ...
                query.categories = {
                    $in: categoryArray
                        ...
                        .map((cat) => createExactMatchRegex(cat))
                };
            }
        }
        ...
        if (formats) {
            ...
            const validFormats = fmtArray.filter((f) => typeof f === 'string' && f.trim().length > 0);
        ...
        const loadArticlesListingData = async () => {
            if (hybridSearchContext) {
                ...
                const fallbackSort = sortMap[sort] || { [defaultDateField]: -1, _id: -1 };
            ...
            const [articles, total] = await Promise.all([
                Article.find(query, hasSearchQuery && useRelevanceMode ? { score: { $meta: 'textScore' } } : undefined)
                    .sort(sortOrder)
                    .skip(skip)
                    .limit(limit)
        ...
        if (redisEligible) {
            ...
            const body = await getOrSetCachedJson(redisKey, ARTICLES_LIST_CACHE_TTL_SECONDS, async () => {
...
export const getArticleById = async (req, res) => {
    try {
        ...
        const detailKey = rawId.length > 0 ? buildApiCacheKey(ARTICLES_DETAIL_CACHE_NS, [rawId]) : null;
        if (detailKey) {
            const cachedBody = await peekCachedParsedJson(detailKey, detailObserve);
            if (cachedBody !== null) {
    ...
    try {
        ...
        if (payloadSize > 1000000) { // > 1MB
            ...
            if (data.images && data.images.length > 0) {
                const imagesSize = data.images.reduce((sum, img) => sum + (img?.length || 0), 0);
        ...
        let publishedAt = status === 'draft' ? null : (data.publishedAt || new Date().toISOString());
...
export const updateArticle = async (req, res) => {
    ...
    try {
        ...
        const clientSentKeys = new Set(req.body && typeof req.body === 'object' ? Object.keys(req.body) : []);
        for (const key of Object.keys(validationResult.data)) {
            if (!clientSentKeys.has(key)) {
                delete validationResult.data[key];
            }
        }
        ...
        if (validationResult.data.images && Array.isArray(validationResult.data.images)) {
            ...
            const existingImagesSet = new Set((existingArticle.images || []).map((img) => img && typeof img === 'string' ? img.toLowerCase().trim() : '').filter(Boolean));
            const imageMap = new Map();
        ...
        const existingStatus = existingArticle.status === 'draft' ? 'draft' : 'published';
        ...
        const targetStatus = requestedStatus || existingStatus;
        if (requestedStatus) {
        ...
        if (updates.tags !== undefined || updates.tagIds !== undefined) {
            ...
            const existingTagIds = (existingArticle.tagIds || []).map((id) => id.toString());
            const mergedIdSet = new Set([
        ...
        const article = await Article.findByIdAndUpdate(req.params.id, { $set: mongoUpdate }, { new: true, runValidators: false } // Disable runValidators for partial updates
        ).lean();
        if (!article)
            return sendNotFoundError(res, 'Article not found');
...
export const deleteArticleImage = async (req, res) => {
    try {
        ...
        if (updatedSupportingMedia.length > 0) {
            ...
            updatedSupportingMedia = updatedSupportingMedia.filter((media) => {
            ...
            if (updatedSupportingMedia.length < beforeCount) {
                ...
                updatedSupportingMedia = updatedSupportingMedia.map((media, index) => ({
            ...
            if (urlMatch && urlMatch[1]) {
                ...
                if (mediaRecord) {
                    ...
                    if (updatedMediaIds.includes(mediaIdString)) {
                        updatedMediaIds = updatedMediaIds.filter((id) => id !== mediaIdString);
 ...
 */
export const getMyArticleCounts = async (req, res) => {
    try {
        // Get current user from authentication middleware
        const currentUserId = req.user?.userId;
        if (!currentUserId) {
...
function normalizeLegacyStreamToFeed(stream) {
    return stream === 'pulse' ? 'market-pulse' : 'home';
}
function getAuthedUserId(req, res) {
...
export const getUnseenFeedCounts = async (req, res) => {
    try {
...
export const markFeedSeen = async (req, res) => {
    try {
        const userId = getAuthedUserId(req, res);
        if (!userId)
            return;
 ...
 */
export const getPulseUnseenCount = async (req, res) => {
    try {
        const userId = getAuthedUserId(req, res);
        if (!userId)
            return;
 ...
 */
export const markPulseSeen = async (req, res) => {
    try {
        const userId = getAuthedUserId(req, res);
        if (!userId)
            return;
 ...
 */
export const getStandardUnseenCount = async (req, res) => {
    try {
        const userId = getAuthedUserId(req, res);
        if (!userId)
            return;
 ...
 */
export const markStandardSeen = async (req, res) => {
    try {
        const userId = getAuthedUserId(req, res);
        if (!userId)
            return;
