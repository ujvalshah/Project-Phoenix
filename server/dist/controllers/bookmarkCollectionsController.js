...
import { sendValidationError, sendNotFoundError, sendConflictError, sendForbiddenError, sendInternalError } from '../utils/errorResponse.js';
import { ensureDefaultCollection, removeBookmarkFromCollection, recalculateCollectionCount } from '../utils/bookmarkHelpers.js';
/**
...
export const updateCollection = async (req, res) => {
    ...
    try {
        ...
        // Apply updates
        const updated = await BookmarkCollection.findByIdAndUpdate(id, { $set: updates }, { new: true }).lean();
        if (!updated) {
            return sendNotFoundError(res, 'Collection not found');
        }
        requestLogger.info({
 ...
 */
export const recalculateCount = async (req, res) => {
    const userId = req.user?.userId;
    const requestLogger = createRequestLogger(req.id || 'unknown', userId, '/api/bookmark-collections');
    try {
        const { id } = req.params;
...
//# sourceMappingURL=bookmarkCollectionsController.js.map