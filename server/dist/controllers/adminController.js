...
import { getMediaQuotaConfig, invalidateMediaQuotaCache } from '../services/mediaQuotaConfigService.js';
import { getDisclaimerConfig, invalidateDisclaimerCache } from '../services/disclaimerConfigService.js';
import { getHomeMicroHeaderConfig, invalidateHomeMicroHeaderCache } from '../services/homeMicroHeaderConfigService.js';
import { getMarketPulseMicroHeaderConfig, invalidateMarketPulseMicroHeaderCache } from '../services/marketPulseMicroHeaderConfigService.js';
...
import { revokeAllRefreshTokensDetailed, upsertUserTokenVersionCache, } from '../services/tokenService.js';
...
const statsCache = new LRUCache(10, 2 * 60 * 1000);
...
export async function getAdminStats(req, res) {
    ...
    const [userAgg, articleAgg, flaggedNuggetsAgg, moderationStats, feedbackAgg] = await Promise.all([
    ...
    const feedbackStats = feedbackAgg.reduce((acc, item) => {
 ...
 */
export async function updateUserSearchCohort(req, res) {
    const adminId = req.userId;
    ...
    const nextSearchCohort = parsed.data.searchCohort === undefined ? undefined : parsed.data.searchCohort;
    ...
    try {
        ...
        const previousSearchCohort = typeof user.appState?.searchCohort === 'string' && user.appState.searchCohort.trim().length > 0
        ...
        getLogger().error({ err: error, adminId, targetUserId }, 'Failed to update user search cohort');
 ...
 */
export async function getMediaLimits(req, res) {
    try {
 ...
 */
export async function updateMediaLimits(req, res) {
    const adminId = req.userId;
    if (!adminId) {
        return res.status(401).json({ message: 'Admin authentication required' });
    }
    ...
    try {
        ...
        const maxStorageBytes = updates.maxStorageMB !== undefined ? updates.maxStorageMB * 1024 * 1024 : current.maxStorageBytes;
        const newDoc = {
            id: 'default',
        ...
        await MediaQuotaConfig.findOneAndUpdate({ id: 'default' }, { $set: newDoc }, { upsert: true, new: true });
        ...
        getLogger().info({ action: 'UPDATE_MEDIA_QUOTA', adminId, previousValue, newValue }, 'Admin updated media quota limits');
        return res.json({
 ...
 */
export async function getDisclaimerSettings(req, res) {
    try {
 ...
 */
export async function updateDisclaimerSettings(req, res) {
    const adminId = req.userId;
    if (!adminId) {
        return res.status(401).json({ message: 'Admin authentication required' });
    }
    ...
    try {
        ...
        await DisclaimerConfig.findOneAndUpdate({ id: 'default' }, { $set: newDoc }, { upsert: true, new: true });
        ...
        getLogger().info({ action: 'UPDATE_DISCLAIMER_CONFIG', adminId, previousValue, newValue }, 'Admin updated disclaimer config');
        return res.json({
 ...
 * Returns effective config (from DB or defaults).
 */
export async function getHomeMicroHeaderSettings(_req, res) {
    try {
 ...
 * Body: { title?, body? }
 */
export async function updateHomeMicroHeaderSettings(req, res) {
    const adminId = req.userId;
    if (!adminId) {
        return res.status(401).json({ message: 'Admin authentication required' });
    }
    ...
    try {
        ...
        await HomeMicroHeaderConfig.findOneAndUpdate({ id: 'default' }, { $set: newDoc }, { upsert: true, new: true });
        ...
        getLogger().info({ action: 'UPDATE_HOME_MICRO_HEADER_CONFIG', adminId, previousValue, newValue, target: 'home_micro_header_config' }, 'Admin updated Home micro-header config');
        return res.json({
 ...
 * Returns effective config (from DB or defaults).
 */
export async function getMarketPulseMicroHeaderSettings(_req, res) {
    try {
 ...
 * Body: { title?, body? }
 */
export async function updateMarketPulseMicroHeaderSettings(req, res) {
    const adminId = req.userId;
    if (!adminId) {
        return res.status(401).json({ message: 'Admin authentication required' });
    }
    ...
    try {
        ...
        await MarketPulseMicroHeaderConfig.findOneAndUpdate({ id: 'default' }, { $set: newDoc }, { upsert: true, new: true });
        ...
        getLogger().info({ action: 'UPDATE_MARKET_PULSE_MICRO_HEADER_CONFIG', adminId, previousValue, newValue, target: 'market_pulse_micro_header_config' }, 'Admin updated Market Pulse micro-header config');
        return res.json({
 ...
 */
async function applyLifecycle(verb, req, res) {
    const adminId = req.userId;
    const targetUserId = req.params.userId;
    if (!adminId) {
        return res.status(401).json({ message: 'Admin authentication required' });
    ...
    try {
        ...
        const shouldRevokeSessions = !wasAlreadyInState && (verb === 'suspend' || verb === 'ban');
        const updateOp = { $set: { status: targetStatus } };
...
export async function revokeUserSessions(req, res) {
    ...
    const reason = parsed.data?.reason;
    try {
        const user = await User.findByIdAndUpdate(targetUserId, { $inc: { tokenVersion: 1 } }, { new: true }).select('tokenVersion auth.email profile.username');
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
