    ...
    try {
        ...
        for await (const article of cursor) {
            ...
            try {
                ...
                // Track changes
                if (tagsResult.changes.trimmed > 0)
                    issueCounts.tagsTrimmed++;
                if (tagsResult.changes.deduplicated > 0)
                    issueCounts.tagsDeduplicated++;
                if (tagsResult.changes.normalized > 0)
                    issueCounts.tagsNormalized++;
                if (tagsResult.changes.fallbackAdded)
                    issueCounts.fallbackTagAdded++;
                const result = {
                    articleId,
                ...
                if (isApplyMode) {
                    ...
                    await Article.updateOne({ _id: article._id }, { $set: updateData });
                    articlesModified++;
...
export { runCleanup };
//# sourceMappingURL=tagCleanupFix.js.map