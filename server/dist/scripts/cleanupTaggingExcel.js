...
function main() {
    ...
    const cleaned = data.map((row, idx) => {
        ...
        for (const part of fmtParts) {
            if (VALID_FORMATS.has(part)) {
                if (!validFmtParts.includes(part))
                    validFmtParts.push(part);
            }
            else {
        ...
        for (const d of domainParts) {
            ...
            if (fixed && fixed !== d) {
                ...
                smartSplitDomain(fixed).forEach(fd => {
                    if (VALID_DOMAINS.has(fd) && !cleanedDomains.includes(fd))
                        cleanedDomains.push(fd);
                });
            }
            else if (VALID_DOMAINS.has(d)) {
                if (!cleanedDomains.includes(d))
                    cleanedDomains.push(d);
            }
            else {
        ...
        for (const s of subParts) {
            if (SUBTOPIC_FIXES[s] !== undefined) {
                ...
                if (replacement === null) {
                    ...
                    if (!cleanedSubs.includes(replacement))
                        cleanedSubs.push(replacement);
                    subtopicFixes++;
                }
            }
            else if (VALID_SUBTOPICS.has(s)) {
                if (!cleanedSubs.includes(s))
                    cleanedSubs.push(s);
            }
            else {
    ...
    XLSX.utils.book_append_sheet(outWb, XLSX.utils.aoa_to_sheet([['Fix Log'], ...fixLog.map(l => [l])]), 'Fix Log');
...
main();
//# sourceMappingURL=cleanupTaggingExcel.js.map