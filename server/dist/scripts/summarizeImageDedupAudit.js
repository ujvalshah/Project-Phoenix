 ...
 */
function parseAuditEntry(lines, startIndex) {
    let i = startIndex;
    const entry = {
        duplicateTypes: [],
        editModeEvents: [],
    ...
    entry.mode = headerMatch[2].toLowerCase();
    i++;
 ...
 */
function calculateGlobalStats(entries) {
 ...
 * Detect risk signals
 */
function detectRiskSignals(entries) {
    const signals = [];
    ...
    const editModeLoss = entries.filter(e => e.mode === 'edit' && e.totalOutputImages < e.totalInputImages && e.movedToSupportingMedia === 0);
    ...
    const mismatchedCounts = entries.filter(e => e.totalInputImages !== e.totalOutputImages && e.duplicatesDetected === 0 && e.imagesRemoved === 0);
    ...
    const highDuplicateConcentration = entries.filter(e => e.duplicatesDetected > 0 && e.duplicatesDetected >= e.totalInputImages * 0.5);
 ...
 */
function calculateModeBreakdown(entries) {
    const createEntries = entries.filter(e => e.mode === 'create');
    const editEntries = entries.filter(e => e.mode === 'edit');
    ...
    const editImagesLost = editEntries.reduce((sum, e) => sum + Math.max(0, e.totalInputImages - e.totalOutputImages - e.movedToSupportingMedia), 0);
    const editPruningRiskCases = editEntries.filter(e => e.imagesRemoved > 0 && e.movedToSupportingMedia === 0).length;
    return {
        create: {
 ...
 */
function identifySuspiciousCases(entries) {
    const suspicious = [];
...
export { main };
//# sourceMappingURL=summarizeImageDedupAudit.js.map