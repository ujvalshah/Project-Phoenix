...
async function backfillArticleLifecycle() {
    ...
    try {
        ...
        const candidates = await Article.find(publishedWithoutPublishedAtQuery)
            ...
            .lean();
        console.log(`[Lifecycle Backfill] Found ${candidates.length} published articles missing publishedAt`);
        if (candidates.length > 0) {
            ...
            console.log(`[Lifecycle Backfill] Updated publishedAt on ${bulkResult.modifiedCount ?? 0} articles`);
        }
...
//# sourceMappingURL=backfillArticleLifecycle.js.map