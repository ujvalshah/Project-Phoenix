...
async function cleanupTagsComprehensive() {
    ...
    try {
        ...
        const tagMergeMap = new Map(); // oldId -> newId
        for (const { canonical, tags } of duplicates) {
            ...
            for (const duplicateTag of duplicatesToMerge) {
                ...
                for (const article of articlesWithDuplicate) {
                    ...
                    await Article.updateOne({ _id: article._id }, { $set: { tagIds: uniqueTagIds } });
        ...
        const existingTagIdSet = new Set(existingTags.map(t => t._id.toString()));
        ...
        for (const article of allArticles) {
            ...
            if (articleTagIds.length > 0) {
                try {
                    ...
                    const normalizedTags = Array.from(new Set(resolvedNames
                        .filter(t => t && t.trim())
                        .map(t => t.trim()))).sort();
                ...
                try {
                    ...
                    // Normalize tags
                    const normalizedTags = Array.from(new Set(articleTags
        ...
        for (const tag of allActiveTags) {
            ...
            // Update usage count
            await Tag.updateOne({ _id: tag._id }, { $set: { usageCount: count } });
...
//# sourceMappingURL=cleanupTagsComprehensive.js.map