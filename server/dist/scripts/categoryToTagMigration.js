 ...
 * - Deduplicates tags (case-insensitive)
 */
function migrateCategoriesToTags(tags, categories) {
 ...
 */
function generateMarkdownReport(report, reportPath) {
    ...
    try {
        ...
        for await (const article of cursor) {
            ...
            try {
                ...
                if (isApplyMode) {
                    await Article.updateOne({ _id: article._id }, { $set: { tags: migratedTags } });
                    articlesModified++;
...
export { runMigration };
//# sourceMappingURL=categoryToTagMigration.js.map