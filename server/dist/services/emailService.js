...
function logMissingResendOnce(reason) {
    if (missingResendWarningLogged)
        return;
 ...
 * No-op if RESEND_API_KEY is not set.
 */
export async function sendAccountExistsEmail(to, loginUrl) {
    if (!resendClient) {
 ...
 * No-op if RESEND_API_KEY is not set.
 */
export async function sendEmailChangedNoticeEmail(to, params) {
    if (!resendClient) {
