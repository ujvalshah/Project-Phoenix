...
import { getRedisClientOrFallback, isRedisAvailable, ensureRedisConnection, } from '../utils/redisClient.js';
...
function hasPipeline(client) {
    return typeof client.pipeline === 'function';
}
/**
 ...
 */
export function isTokenServiceAvailable() {
    return true;
}
// ============================================================================
...
export async function blacklistToken(token, expiresInSeconds) {
    ...
    try {
        ...
        await client.setEx(`${PREFIX.BLACKLIST}${tokenHash}`, Math.max(1, expiresInSeconds), '1');
        return true;
    }
    catch (error) {
        const logger = getLogger();
...
export async function isTokenBlacklisted(token) {
    ...
    const strict = (() => {
        try {
            return getEnv().STRICT_TOKEN_REVOCATION === true;
        }
        catch {
            return false;
        }
 ...
 */
export async function invalidateUserTokenVersionCache(userId) {
    if (!isRedisAvailable()) {
        return;
    }
    try {
 ...
 */
export async function upsertUserTokenVersionCache(userId, tokenVersion) {
 ...
 */
export async function getUserTokenVersionForAuth(userId) {
    let enabled = true;
    let ttlSeconds = 120;
    try {
        const env = getEnv();
 ...
 */
export async function storeRefreshToken(userId, refreshToken, deviceInfo, ipAddress) {
...
export async function validateRefreshToken(userId, refreshToken) {
    ...
    try {
        ...
        const isConnectionError = errMsg.includes('Connection') ||
 ...
 */
export async function rotateRefreshToken(userId, oldRefreshToken, deviceInfo, ipAddress) {
    const logger = getLogger();
    // Try real Redis first, fall back to in-memory store
    await ensureRedisConnection();
    try {
        ...
        if (hasPipeline(client)) {
            ...
            pipeline.setEx(`${PREFIX.REFRESH_ROTATED}${oldHash}`, REFRESH_ROTATED_TTL, userId);
            const results = await pipeline.exec();
            // CRITICAL FIX: Validate pipeline results - pipelines are NOT atomic
            ...
            await client.setEx(`${PREFIX.REFRESH_ROTATED}${oldHash}`, REFRESH_ROTATED_TTL, userId);
            // CRITICAL FIX: Verify old token was deleted (security check)
            const oldTokenStillExists = await client.exists(refreshKey);
            if (oldTokenStillExists) {
                logger.error({
 ...
 */
export async function revokeAllRefreshTokensDetailed(userId) {
    if (!isRedisAvailable()) {
 ...
 */
export async function checkRefreshTokenReuse(refreshToken) {
    await ensureRedisConnection();
    if (!isRedisAvailable())
        return null;
    try {
 ...
 * version bumps.
 */
function fingerprintUserAgent(ua) {
    if (!ua)
        return 'unknown';
 ...
 */
export function isSessionFingerprintMismatch(stored, current) {
 ...
 */
export async function closeTokenService() {
