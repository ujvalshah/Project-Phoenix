 ...
 */
async function safeInsertMany(model, docs, label) {
    if (docs.length === 0)
        return [];
    try {
...
function buildDigestPayload(articles) {
...
async function sendWebPushToSubscription(sub, payload) {
    ...
    try {
        ...
        await PushSubscription.updateOne({ _id: sub._id }, { $set: { lastSuccessAt: new Date(), invalidatedReason: undefined, failureCount: 0 } });
...
async function processFanOut(job) {
    const logger = getLogger();
    const { articleId, publishEventId } = job.data;
    ...
    const inAppEligibleIds = [];
    const pushSuppressedForQuiet = new Set();
    for (const user of instantUsers) {
        const prefs = user.preferences?.notifications;
        const catFilter = prefs?.categoryFilter || [];
        const passesCategory = catFilter.length === 0 ||
    ...
    for (const batch of batches) {
        const results = await Promise.allSettled(batch.map(sub => sendWebPushToSubscription(sub, payload)));
        for (const [idx, result] of results.entries()) {
            if (result.status === 'fulfilled') {
                const currentSub = batch[idx];
    ...
    const insertedNotifications = await safeInsertMany(Notification, notificationDocs, 'in-app fan-out');
    ...
    if (deliveryDocs.length > 0) {
        await safeInsertMany(NotificationDelivery, deliveryDocs, 'delivery-ledger');
    }
    logger.info({
...
async function processDailyDigest(job) {
    const logger = getLogger();
    const enabled = await isNotificationsEnabled();
    if (!enabled) {
    ...
    const insertedDigestDocs = await safeInsertMany(Notification, notificationDocs, 'in-app digest');
    ...
    if (deliveryDocs.length > 0) {
        await safeInsertMany(NotificationDelivery, deliveryDocs, 'delivery-ledger-digest');
    }
    logger.info({
...
export async function initNotificationService() {
    ...
    try {
        ...
        if (indexes.some((i) => i.name === 'endpoint_1' && i.unique)) {
    ...
    const scheduleWithTimeout = async (name, opts) => {
        const addPromise = notificationQueue.add(name, {}, opts);
        const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error(`BullMQ ${name} schedule timed out after 5s`)), 5000));
        try {
 ...
 */
export async function onArticlePublished(articleId, publishEventId = Date.now()) {
    const logger = getLogger();
    if (!notificationQueue) {
        ...
        const eligibleIds = instantUsers
            ...
            return (catFilter.length === 0 ||
                catFilter.some((cat) => articleTagNames.includes(cat.toLowerCase())));
        })
            .map((u) => u._id.toString());
        if (eligibleIds.length === 0)
            return;
        await safeInsertMany(Notification, eligibleIds.map((userId) => ({
            userId,
            type: 'new_nugget',
            title: payload.title,
            body: payload.body,
    ...
    try {
        await notificationQueue.add('fan-out', { articleId, publishEventId }, {
 ...
 */
export async function closeNotificationService() {
    const logger = getLogger();
    try {
