...
function isImageUrl(urlString) {
    try {
        ...
        if (hostname.includes('images.ctfassets.net') ||
 ...
 */
function detectContentType(url, domain) {
    const pathname = url.pathname.toLowerCase();
    const hostname = url.hostname.toLowerCase();
    // YouTube
 ...
 */
function shouldAutoGenerateTitle(contentType, urlString) {
 ...
 */
async function tier0_6_youtube(urlString, domain) {
 ...
 */
async function tier1(urlString, isAdmin) {
    // Check conditions
    if (!MICROLINK_ENABLED)
        return null;
    if (MICROLINK_ADMIN_ONLY && !isAdmin)
        return null;
 ...
 */
async function tier2(urlString) {
    try {
 ...
 */
function enrichPlatformSpecific(nugget, url, domain) {
    // YouTube specific
    if (domain.includes('youtube.com') || domain.includes('youtu.be')) {
 ...
 */
export async function fetchUrlMetadata(urlString, options = {}) {
    ...
    if (!checkTimeout() && (domain.includes('youtube.com') || domain.includes('youtu.be'))) {
        try {
            const enrichment = await withTimeout(tier0_6_youtube(urlString, domain), 1000 // 1 second timeout
            );
            if (enrichment) {
    ...
    if (shouldTryMicrolink) {
        try {
            const enrichment = await withTimeout(tier1(urlString, isAdmin), TIER_1_TIMEOUT);
            if (enrichment) {
    ...
    if (shouldTryOG && !checkTimeout()) {
        try {
            const enrichment = await withTimeout(tier2(urlString), TIER_2_TIMEOUT);
            if (enrichment) {
                // PHASE 2: Title from metadata is stored for SUGGESTION ONLY
                // Frontend will transform it to previewMetadata.title and never auto-populate
                // Title generation must happen ONLY when the user explicitly clicks a "Generate title" button
    ...
    if (needsImageProbe && baseNugget.media && !checkTimeout()) {
        try {
            const dimensions = await withTimeout(tier3(baseNugget.media.src), TIER_3_TIMEOUT);
            if (dimensions) {
