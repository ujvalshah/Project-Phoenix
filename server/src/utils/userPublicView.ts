...
export interface PublicUserView {
  ...
  profile: {
    ...
    company?: string;
    /**
     * `profile.location` is the user-set free-text city/region they chose to
     * publish (e.g. "Bangalore, India"). It is intentionally separate from the
     * structured `profile.city` / `profile.country` / `profile.pincode` fields,
     * which are PII and never published.
     */
    location?: string;
    website?: string;
    twitter?: string;
 ...
 */
export function toPublicUserView(user: UserLike | null | undefined): PublicUserView | null {
  if (!user) return null;

  const plain: Record<string, unknown> =
    typeof user.toObject === 'function' ? user.toObject() : (user as Record<string, unknown>);

  const rawId = (plain._id ?? plain.id) as { toString(): string } | string | undefined;
  if (rawId === undefined || rawId === null) return null;
  const id = typeof rawId === 'string' ? rawId : rawId.toString();
  if (!id) return null;

  const role = plain.role === 'admin' ? 'admin' : 'user';
  const profile = (plain.profile ?? {}) as Record<string, unknown>;
