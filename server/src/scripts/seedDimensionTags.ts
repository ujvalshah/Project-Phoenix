...
import type { TagDimension } from '../models/Tag.js';
import mongoose from 'mongoose';

// ─── Tag Taxonomy Definition (v2 — flat, no parent-child) ──────────────────
...
async function seedDimensionTags(dryRun: boolean): Promise<void> {
  let created = 0;
  let updated = 0;
  let skipped = 0;
  ...
  async function upsertTag(seed: TagSeed, dimension: TagDimension): Promise<void> {
...
async function deprecateOldTags(dryRun: boolean): Promise<void> {
